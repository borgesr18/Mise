...
import { Recipe } from '@prisma/client'; // Importa o tipo Recipe gerado pelo Prisma
...
export default function EditRecipeForm({ recipe }: { recipe: Recipe }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/recipes/${recipe.id}`); // Volta para a página de detalhes
      router.refresh();
    } catch (error) {
      console.error(error);
      ...
      <form onSubmit={handleSubmit} className="p-6 bg-white rounded-lg shadow-md space-y-4">
        ...
        <div className="flex gap-4">
          ...
          <div className="flex-1">
            ...
            <input type="text" name="yieldUnit" id="yieldUnit" value={formData.yieldUnit} onChange={handleChange} required className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm" />
          </div>
        </div>
